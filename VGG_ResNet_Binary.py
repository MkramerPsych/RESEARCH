...
################################################################################
    ...
    data_dir = 'data/VGG_Subset_600000' #PATH TO DATA FROM CURRENT DIRECTORY
...
################################################################################
    ...
    def train_model(model, criterion, optimizer, scheduler, num_epochs=25): #25 EPOCH DEFAULT
        ...
        for epoch in range(num_epochs): #RUN NUM_EPOCH TIMES

            #FORMAT READOUT#
            print('Epoch {}/{}'.format(epoch + 1, num_epochs))
            print('-' * 10)

            #CONTROL PHASE OF MODEL#
            for phase in ['train', 'val']:
                ...
                #TRAINING LOOP#
                for inputs, labels in tqdm(dataloaders[phase], unit_scale=dataloaders[phase].batch_size, desc='Epoch {}, {} phase'.format(epoch + 1, phase)): #PROGRESS TRACKING
                    inputs = inputs.to(device)
                    labels = labels.to(device)

                    if num_epochs == 1:
                        grid = torchvision.utils.make_grid(inputs,normalize=False,scale_each=False)
                ...
                #TENSORBOARD LOGGING#
                if phase == 'train':
                    writer.add_scalar('loss/training_loss',epoch_loss,epoch+1)
                    writer.add_scalar('acc/training_acc',epoch_acc,epoch+1)
                if phase == 'val':
                    writer.add_scalar('loss/validation_loss',epoch_loss,epoch+1)
                    writer.add_scalar('acc/validation_acc',epoch_acc,epoch+1)
